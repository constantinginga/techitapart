...
//        categoryList.setAll(model.getAllCategory());
    ...
    public void chooseImage(Stage stage) {
        ...
        try {
            filePath = Files.copy(filePath.toPath(), new File("server\\resources\\images\\" + filePath.getName()).toPath()).toFile();
        } catch (Exception e) {
            e.printStackTrace();
        }
        String extension = "";
        ...
        filePath.renameTo(new File("server\\resources\\images\\" + fileName));
        Platform.runLater(() -> {
            try {
                BufferedImage bufferedImage = ImageIO.read(new File("server\\resources\\images\\" + fileName));
