...
public class ModelManager implements Model
{
  ...
  public ModelManager()
  {
    ...
    map = new HashMap<>();
       /* user = new User(persistence.getfName(), persistence.getlName(),
            persistence.email(), persistence.getUsername(),
            persistence.getPassword());*/
    //    user = new User(persistence.getUser().getfName(),
    //        persistence.getUser().getlName(), persistence.getUser().getEmail(),
    //        persistence.getUser().getUserName(),
    //        persistence.getUser().getPassword());
  ...
  @Override public UserProfile registerUSer(String fName, String lName,
  ...
  // how will this login method work in server client system?
  @Override public UserProfile login(String username, String password)
  {
   ...
   * Category
   **/
  @Override public ArrayList<String> getAllCategory()
  {
    return categories;
  }
  ...
  @Override public Product getProduct(String id, String categoryName)
  {
  ...
  @Override public ArrayList<Product> getAllProductsInCategory(
      String categoryName)
  {
    return (categoryName.equals("General")) ?
        getAllProducts() :
        map.get(categoryName).getAllProduct();
  }
  ...
  @Override public File getImage(String url)
  {
    return new File("server\\resources\\images\\" + url);
  }

  @Override public void uploadImage(File file, String fileName)
  {
    try
    {
      file = Files.copy(file.toPath(),
          new File("server\\resources\\images\\" + file.getName()).toPath())
          .toFile();
  ...
  @Override public ArrayList<Product> searchForProducts(String productName)
  {
  ...
  @Override public void updateProductQuantity(String id, int quantity,
      String categoryName)
  {
  ...
  @Override public void updateProductPrice(String id, double price,
      String categoryName)
  {
   ...
   **/
  // how will this login method work in server client system?
  @Override public void addProductToCart(Product product, int quantity)
  {
    userProfile.addProductToCart(product, quantity);
    persistence.addProductToCart(Integer.parseInt(product.getId()), quantity,
        userProfile.getUsername());
  }

  // how will this login method work in server client system?
  @Override public void updateCartItemQuantity(CartItem cartItem, int quantity,
      String username)
  {
    userProfile.updateCartItemQuantity(cartItem, quantity);
  }

  @Override public void removeProductFromCart(CartItem cartItem,
      String username)
  {
  ...
  // how will this login method work in server client system?
  @Override public void buy(String username)
  {
    Order order = new Order(persistence.addOrderDB(username),
        userProfile.getUsername());
    ...
    for (CartItem cartItem : userProfile.getAllCartItem())
    {
      ...
      persistence.decreaseProductQuantity(cartItem.getProduct().getId(),
      ...
      String categoryName, String userName)
  {
    ...
    property.firePropertyChange("quantity", product.getId(),
        product.getTotal_quantity() - quantity);
    persistence.decreaseProductQuantity(product.getId(), quantity);
    getCategory(categoryName)
        .decreaseProductQuantity(product.getId(), quantity);
  }
